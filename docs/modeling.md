...
| **事件触发** | ±5% 简化 A | **±3% event_price + event_invalidation 硬失效位击穿** |
...
| **运营节奏** | 每日 16:00 完整 A + 持仓期 4h 健康检查 + 异动 ±5% | **保持每日 16:00 完整 A + 持仓期 4h 健康检查 + 异动改 ±3%** |
...
| 宏观类 | 4 个(删 5) | Yahoo / FRED |
| 机构 / 市场结构类 | 2 个(全新) | CoinGlass / 自算 |
| 事件 / 价格类 | 2 个 | 手动 YAML / 自算 |
| **合计** | **43 个** | |

> **v1.4 注**:v1.3 文档摘要写"44 因子",实际明细列表 43 个。v1.4 保持 v1.3 实施现状,以代码 `config/data_catalog.yaml` 为权威。任何因子增减必须先改 v1.4 文档再改代码。
...
│  Binance / CoinGlass / Glassnode / Yahoo / FRED │
└─────────────────────────────────────────────────┘
                    ↓
...
│                                                   │
│  不做(交给 AI):                                  │
│  - regime / stance / grade / direction 判断       │
│  - macro_stance 判断                              │
│  - trade_plan 生成                                │
└─────────────────────────────────────────────────┘
                    ↓
┌─────────────────────────────────────────────────┐
│  Step 2: AI 介入层(指数退避重试 + 短路依赖)      │
│                                                   │
│  L1 (AI) ──┬──► L2 (AI) ──┬──► L3 (AI) ──┐       │
│            │              │              │       │
│            ├──► L4 (规则) ┤              ▼       │
...
### 3.3.4 L4 风险评估(继承 v1.3,纯规则)

L4 仍然是规则层,**不接 AI**(除了 L5 AI loopback 修正 position_cap)。

L4 输出:
- hard_invalidation_levels(止损价候选)
- position_cap_base
...
1. **首先决定 mode**(三选一):
   ...
   - 如果 active_thesis 为 null + 不在冷却 + L3 grade=C 或 none → 输出 silent(等下一档机会)
...
3. **mode=new_thesis 时必须做**:
   ...
   - 配套 confidence_score 0-100(必须跟 L3 grade 对齐:A→80-100 / B→60-80 / C→40-60 / none→不创建)
...
```json
{
  ...
  "hard_constraint_activation_review": {
    "validator_7_confidence_capped_rate": "5/7 days",
    "position_cap_avg": 0.42,
    "thesis_lock_blocks": 1,
    "channel_c_uses": 0,
    "review_pending_triggers": 0,
    "evaluation": "validator_7 触发率偏高,可能数据降级阈值过严",
    "suggested_action": "考虑放宽 stale_data_policy 中的 freshness 阈值"
  },
...
**Validator 5**:`grade=none` 时不允许创建 thesis
- 失败处理:强制 mode=silent,thesis 不创建
- meta 记录:`activations.validator_5`
...
- 满足 thesis 创建条件(无 active + 不在冷却 + 不在熔断 + L3 grade ∈ {A, B})但 master 输出 silent
...
存入 `strategy_runs.constraint_activations_json` 字段。
...
| 触发 | AI 方案 | 频率 | 单次成本 |
|---|---|---|---|
| **价格异动 ±3%(event_price)** | 简化 A(应急 AI) | 0-2 次/天平均 | ~$0.10 |
| **硬失效位击穿(event_invalidation)** | 规则平仓(无 AI)+ 推送告警 | 立刻 | $0 |
...
跟 v1.3 估算 $13-15 接近,v1.4 略增(±3% 比 ±5% 触发更频繁 + 周复盘新增)。
...
event_trigger:
  price_pct: 0.03                 # ±3%(v1.4 改,v1.3 是 0.05)
  event_cooldown_seconds: 7200
  skip_if_recent_scheduled_seconds: 1800
```
...
| **1.10-J** | scheduler.yaml / base.yaml 配置统一 + 旧逻辑清理 | 1 天 | 1.10-I |
