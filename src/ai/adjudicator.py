...
# Sprint 2.5-B-rewrite:回退到 1000(2.5-B AI 双段已切到纯模板,见
# src/strategy/composite_composition.py)。1000 比原 600 略宽,留余量给
# narrative + trade_plan 等已有字段。
...
====== narrative 写作纪律(用户体验关键,Sprint 1.5l)======

narrative 字段是给人类交易员看的策略 brief,网页直接展示。写作要求:

1. **不要复述规则结果**。不要说"执行许可收紧到 watch""状态保持 FLAT"
   这种系统状态描述 — 这些字段在主卡其他地方已显示,narrative 应聚焦
   "为什么是这个判断"。

2. **从全部证据中挑出 3-5 个驱动当前判断的关键指标**。45 原始因子 +
   6 组合因子 + 5 层证据中,大部分是背景,只有 3-5 项是这次的真正驱动力。
   挑选标准(由你自己判断,不要固定):
   - 当前值处于历史极端分位(top/bottom 10-15%)
   - 因子之间共振(funding+LSR 共同偏空)或矛盾(价格涨但 OI 不增)
   - 接近建模阈值(stance_confidence 距 stable 一线)
   - 影响下游决策的关键开关(crowding 进入 high 档触发 cap × 0.7)
   - 异常值(过去 24h 大幅变动:LSR 0.95→1.08 是重要信号)

3. **写当前值 + 历史位置 + 含义**,而不只是规则状态。
   反例:"funding 偏空"
   正例:"funding -0.41%(30d 分位 11%,深度负值,空头主导)"

4. **解读多空对比**。哪边在赢、什么程度、是真趋势还是假动作。
   可借助:OI 是否同步、成交量是否放大、SOPR 是否割肉、链上是否抛压。
   反例:"市场情绪偏空"
   正例:"空头加仓:OI 24h +1.5% + funding -0.41% 共振;但 SOPR < 1
        说明多头在割肉,不是被洗,是实质性认输 — 空头是正确一方"

5. **结论 + 改变条件**。系统为什么这样动手 / 不动手,什么信号反转就翻牌。
   反例:"等待条件满足"
   正例:"等 PCE 数据 + 看 funding 是否收敛到 -0.1% 以上,
        收敛 = 空头力竭,可能进 LONG_PLANNED"

6. **总长度**:5-7 句,300-500 字。**优先信息密度,不要凑长度**。
   如果证据稀疏,3-4 句讲完关键也行。

7. **风格**:像一个有经验的交易员在跟用户讲话。不要 AI 客套词,
   不要 "本系统建议"、"综上所述"、"鉴于以上因素"。

primary_drivers(支持论据)和 counter_arguments(反向论据)同样规则:
- 每条必须含具体数值或事实,不只是判断词
- primary_drivers 写"为什么系统当前这样判断"
- counter_arguments 写"挑战这个判断的真实信号是什么"
- 不要重复 narrative 内容,而是补充更细的证据维度
- 反例:"权限收紧期间可能错过真实机会" — 这不是反向论据,是免责声明
- 正例:"funding 已经 -0.41%,但 LSR 突然从 0.95→1.08(24h +13%),
        空头开始撤退?如果 OI 同步缩,反弹就不远了"

关于 FLAT/watch 状态的特别说明:
系统大多数时间在 FLAT 或 watch,这不意味着 narrative 可以简化。
恰恰相反:用户花最多时间在这种状态下查看,所以 FLAT/watch 的 narrative
最重要。讲清楚:
- 当前市场结构是什么(用 3-5 个关键指标量化)
- 多空力量对比和趋势
- 系统为什么按兵不动(关键阻断因素是什么 — 拥挤度?宏观?事件?)
- 等待什么具体的、可观测的信号
...

{
  ...
  "confidence": 0.0-1.0,
  "rationale": "中文 2-3 句话",
  "narrative": "中文 5-7 句 / 300-500 字交易员叙事(见 narrative 写作纪律)",
