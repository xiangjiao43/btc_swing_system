...
| 文件 | 说明 |
|---|---|
| `config/data_catalog.yaml` | 登记 6 个 Glassnode + 4 个 FRED Layer A 第一批因子 |
| `src/data/collectors/glassnode.py` | 新增 LTH/STH SOPR、盈利/亏损供给比例、交易所余额、交易所净头寸变化 fetcher |
...
|---|---|---|---|---:|---|---|
| LTH SOPR | Glassnode | config_only → already_collected | `config/data_catalog.yaml` 已有 `/v1/metrics/indicators/sopr_lth`；本轮新增 `GlassnodeCollector.fetch_lth_sopr()` | true | low | 与现有 Glassnode scalar `v` 序列风格一致 |
| STH SOPR | Glassnode | config_only → already_collected | `config/data_catalog.yaml` 已有 `/v1/metrics/indicators/sopr_sth`；本轮新增 `fetch_sth_sopr()` | true | low | 与 LTH SOPR 同类 |
...
| Percent Supply in Loss | Glassnode | not_found → already_collected | 官方文档 supply endpoint `/v1/metrics/supply/loss_relative`；本轮新增 catalog + collector | true | low | scalar 序列，可复用 `onchain_metrics` |
...
| Exchange Net Position Change | Glassnode | not_found → already_collected | 官方文档 distribution endpoint `/v1/metrics/distribution/exchange_net_position_change`；本轮新增 catalog + collector | true | low | scalar 序列，可复用 `onchain_metrics` |
...
- Glassnode: `lth_sopr`, `sth_sopr`, `percent_supply_in_profit`, `percent_supply_in_loss`, `exchange_balance`, `exchange_net_position_change`
...
新增 6 个 fetcher，全部复用 `_fetch_series()`：
...
本轮把已经接入的 10 个因子从 `_UNAVAILABLE_MODEL_FACTORS` 和 normalizer 的 critical missing 集合中移除。
...
- LTH/STH SOPR：长期/短期持有人实现盈亏状态。
- Percent Supply in Profit/Loss：顶部过热或深度低估。
- Exchange Balance / Net Position Change：交易所潜在卖压或筹码流出。
...
- 新增 Glassnode endpoint 已按官方路径和项目 collector 风格接入，但真实生产 API 是否返回完整数据，以生产采集日志为准。
...
2. 若 `critical_unavailable_count` 下降但 `confidence_cap` 仍是 medium，下一批优先验证 `RHODL / Reserve Risk / Puell / LTH Net Position Change / Real Yield`。
