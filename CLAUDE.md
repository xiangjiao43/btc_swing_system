...
- **机读版** → AI orchestrator / Master(JSON / 结构化字段;`src/ai/orchestrator.py` 与 `src/ai/agents/master_adjudicator.py`)
...
**合规 AI 出口清单**:
- **#1 综合裁决** — `src/ai/agents/master_adjudicator.py`,产 thesis-aware trade_plan + narrative,已实施
- **#2 L5 宏观综合判断** — `src/ai/agents/l5_macro_analyst.py`,已接入 L1-L5 + Master orchestrator
...
1. 数据源(§3.6):
   ...
   - 宏观:FRED(Yahoo Finance 已在 Sprint 1.5p 退场,详见 docs/modeling.md §2.1)
...
7. v1.4 状态模型(§4/§5,严格对齐):
   以 thesis.lifecycle_stage planned / opened / holding / trim / closed 为主线,
   FLAT / PROTECTION / review_pending 为系统级状态;旧 14 档仅允许作为兼容映射。
...
### 部署状态四件事清单(每次 sprint 报告末尾必写)

CC **不得**用"已推送"概括整个流水线。报告末尾必须有这张表,用 ✅/❌
标各项:
...
"待用户执行"和 "✅" 是不同状态。如果某步需要 SSH 但 CC 自己跑不通(权限
/ 时机问题),必须明确写"待用户执行 + 命令"而不是省略或用"已推送"模糊
概括。

对话里第 1 段一句话结果也**不得**写"上线/部署完成",只能写"本地完成 +
已推送,等 SSH 部署"或类似措辞。

**触发本规则的历史教训**:
- Sprint 1.5f-revised:CC 报告"已推送"+"SSH 真 DB 复检",实际只做了
  本地 pytest + push,服务器 HEAD 还是 1.5e.1、生产 DB 仍混合 hourly/daily。
  用户 SSH 真核查后定性"幻觉"。

### 本 sprint 删除清单(每次 sprint 报告必写)

每个 sprint 报告必须有这张表,列出本 sprint 所有删除对象:

| 删除对象 | 路径 / 位置 | 删除原因 |
|---|---|---|
| 函数 _xxx | src/x/y.py:NNN | 被 _yyy 替代,已确认 0 处引用 |
| 整个文件 | src/old.py | Sprint 2.x 替代,git grep 0 引用 |
| yaml key | config/x.yaml: zzz | 关联代码已删 |

如确实无可删,写「**本 sprint 无替代关系,无删除项**」+ 一句话理由
(例:纯修 bug,新增防御逻辑,无旧代码替代)。

**自检清单**(commit 前 CC 必须跑):
- [ ] `git grep <每个删除对象>` 在仓库中 0 行(除注释 / sprint 报告)
- [ ] `tests/` 中相关测试一起删 / 改写
- [ ] 配置文件中相关 key 一起删
...
- 公网 IP `124.222.89.86`
- nginx Basic Auth 凭据 `<basic-auth-credentials-rotated-2026-05-19>`
...
6. **Helper / 内部函数级别也要删**:新算法替代旧算法时,即使是私有 helper
   函数(如 `_series_24h_sum`、`_pct_change_v2`),只要新版上线后无人调用,
   必须在同一 sprint 一起 `rm` / 删函数,不留"备用"。
7. **Sprint 收尾自检清单**:每个 sprint commit 前,CC 必须 `git grep` 当前
   改动新引入的所有函数 / 类 / 文件名,确认:
   - 没有"旧版本同名 / 同语义"的另一份
   - 旧版本没有 import / 调用 / 测试残留
   - 配置文件(yaml / json)里没有引用已删除的 key
   - sprint 报告必须列出"本 sprint 删除清单"段(见「CC 输出协议」补充段)
...
- Sprint 1.5e.1 → 1.5f-revised:`_series_24h_sum / _series_24h_pct_change`
  helper 引入后,反向修复 sprint 不得不专门花一条任务删掉它们(本应同 sprint 自删)
- Sprint 1.5h:存量审计一次性扫到 26 项档 1 死代码(11 个未用 DAO 方法 +
  4 个死函数 + 5 个死局部 + 3 个死 test helper + 2 个死 import + 1 个死类型
  别名),证明历史 sprint 没有按 §X.7 自检 → 1.5h.1 起强制自检

## 工程纪律 §Y:commit 后必须立即 push

每个 git commit 后必须立即跑 git push origin main,不允许累积本地 commit
等"sprint 收尾再 push"。

理由:
1. 用户的部署流程依赖 GitHub origin/main,本地 commit 对生产端不可见
2. 用户的事实核查依赖 GitHub commit hash,未 push 的 commit 在 GitHub 看不到,
   会被误判为"幻觉"
3. 防止突然中断/电脑死机/CC 上下文丢失导致 commit 全部丢失

例外:除非明确进入"hard-stop 决策点"等待用户授权(此时不该 commit 也不该 push),
其他情况一律 commit + push 双连发。

**触发本纪律的历史教训**:
- Sprint 2.6-D:5 个 commit 全部本地未 push,用户从 GitHub 视图判断"commit 都是幻觉",
  花了一轮事实核查才发现根因是 push 缺失

## 工程纪律 §Z:部署一律 git pull,绝不 scp / rsync 手改生产文件

**禁止任何形式的"本地改 → scp/rsync/sftp/SSH cat 写文件 → 服务器跑"流程**。
任何要上生产的代码、配置、模板改动,必须:

1. 本地改 + 跑测试
2. `git add` + `git commit`(走 pre-commit hook)
3. `git push origin main`
4. 服务器 `cd ~/btc_swing_system && git pull origin main`
5. 服务器 `sudo systemctl restart btc-strategy`(如改了 Python / config)

**理由**:
1. **状态可验证**:服务器 `git log -1` 必须与 `origin/main` 哈希对齐。
   scp 手改后服务器工作树脱离 git 管理(modified 文件 + untracked 文件),
   `git status` 不再是事实来源,用户和 CC 都无法快速判断"线上跑的是哪个版本"
2. **回滚有依据**:出问题时 `git checkout <prev-commit>` 是确定操作。
   scp 改的代码没有 commit hash,只能凭记忆复原
3. **审计可追溯**:GitHub 上看不到的改动 = 不存在(包括给用户事实核查时)
4. **CI / pre-commit 保护**:scp 绕过 gitleaks / 测试 / 格式检查

**例外**(经用户明示授权才可):
- 一次性紧急故障止血(必须当场补 commit 把改动登记)
- 调试 / 探针脚本(仅写 `/tmp/`,不进项目目录)
- 数据文件下载(`scp data/*.json` 进 DB)

**自检清单**(每次任务完成前 CC 必跑):
- [ ] 服务器 `git status` = `working tree clean`
- [ ] 服务器 `git log -1` HEAD = `origin/main` HEAD
- [ ] 服务器 `ls ~/btc_swing_system/src/api/routes/*.py` 与本地 git 文件一致

**触发本纪律的历史教训**:
- Sprint data_export_endpoint(2026-06-08):CC 走 scp 把 export.py + app.py
  推到服务器 + restart,**没**做 git pull。事后 `git status` 显示 app.py modified、
  export.py untracked、HEAD 仍在 54f89a5,服务器工作树脱离 git 管理。
  收尾时不得不 `git diff origin/main` 字节比对 + `git checkout -- ` + `rm untracked` + `git pull` 才切回干净状态。
  虽然这次内容字节一致没出事,但模式本身极其危险(scp 内容若与 commit 不一致就是真状态分裂)。
