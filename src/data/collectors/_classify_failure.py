...
failure_reason 5 桶:
  quota_exceeded   HTTP 403/429 + 中转站配额话术(quota / rate limit / 配额)
  network_error    requests.ConnectionError / Timeout / DNS / 其他网络层
...
def classify_fetch_failure(exc: BaseException) -> Tuple[str, str]:
    ...
    顺序:network_error → quota_exceeded(403/429 或 quota 话术)→
    api_error(其他 4xx/5xx)→ parse_error(JSON / schema)→ unknown。
    """
