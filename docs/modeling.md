...
> **⚠️ 历史设计 — 已于 2026-05-19 退役**
>
> 本节描述的 9 档 cycle_position 是 v1.4 建模时(Layer A 尚不存在)唯一保留的组合因子。
> Layer A 独立子系统上线后,"大周期判断"职责完全移交 Layer A 6 阶段。
> 本节保留作为历史设计参考,但 v1.4-cleanup 后系统不再使用 cycle_position 9 档。
>
> 当前大周期判断的权威来源:Layer A `LayerACycleAdjudicator` 的
> `official_stage_recommendation`(6 阶段:bear_bottom / recovery / bull_main /
> bull_late / top_distribution / bear_decline)。
>
> 详见:docs/cc_reports/sprint_layer_b_factor_cleanup.md
...
> **2026-05-19 更新**:CyclePosition 输入已移除,L2 不再依赖大周期判断;
> Layer A 6 阶段已独立负责大周期。L2 输出 schema 同步删除 `long_cycle_context`
> 字段。Layer B 反模式从 5 类降为 4 类(删除 `is_against_long_cycle`)。
> 详见:docs/cc_reports/sprint_layer_b_factor_cleanup.md
...
1. **首先决定 mode**(三选一):
   ...
   - 如果 active_thesis 为 null + 不在冷却 + 不在熔断 + L3 grade ∈ {A, B} → mode = "new_thesis"
     创建新 thesis(必须给完整 trade_plan)
   - 如果 active_thesis 为 null + 不在冷却 + L3 grade=C → mode = "silent_cooldown" 或观察型说明
     注意:C 级为观察型 / 低质量波段机会,默认不创建 thesis、不进入虚拟账户、不触发订单
...
3. **mode=new_thesis 时必须做**:
   ...
   - 配套 confidence_score 0-100(必须跟 L3 grade 对齐:A→80-100 / B→60-80 / C→不创建 thesis / none→不创建)
...
- **`grade=C` 时:观察型机会,不允许创建 thesis**(强制 mode=silent,trade_plan=null)
- 失败处理:强制改为合法 permission 或 silent,notes 添加 `permission_overridden_for_grade`
...
- 不在 PROTECTION
- 不在 review_pending
- **L3 grade ∈ {A, B}**(C 级只作为观察 / future watch,不创建 thesis)
...
### 4.3.6 FLIP_WATCH 冷却时长(2026-05-19 更新)

历史上 FLIP_WATCH 冷却时长会根据 `composite_factors.cycle_position` 的 9 档
判断做乘数调整(mid_bull/mid_bear × 1.3 拉长,late_bull/late_bear/accumulation/
distribution × 0.7 缩短)。

自 Layer A 独立子系统上线、Layer B 不再做大周期判断后,`cycle_position` 9 档
因子已退役(详见 docs/cc_reports/sprint_layer_b_factor_cleanup.md)。

现行规则:**FLIP_WATCH 冷却时长统一为 base 时长(18-96h,按反手通道档位),
不再因大周期阶段调整。**

设计权衡:
- 优点:状态机时序与大周期判断解耦,Layer A 阶段变化不再连锁影响 Layer B 反手节奏
- 牺牲:失去"牛市中段反手谨慎、牛市末段反手敏捷"这种与大周期联动的时序调节

未来如果需要恢复大周期对 FLIP_WATCH 的影响,建议读 Layer A 6 阶段输出
(`official_stage_recommendation`)作为乘数源,而不是恢复 9 档 cycle_position。
